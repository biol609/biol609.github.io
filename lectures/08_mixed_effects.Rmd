...
anova(plants_mer, type=2)
```

## Satterhwaite Approximation with lmerTest
```{r anova_mer_ws, echo=TRUE}
library(lmerTest)
...
anova(plants_mer_testme, type=2)
```

## KR Approximation with lmerTest
```{r anova_me_krr, echo=TRUE}
anova(plants_mer_testme, ddf="Kenward-Roger", type=2)
```

Note that DenDF are 6. Were 8 with Satterthwaite.

## Wald Tests with lmerTest
```{r wald_me_krr, echo=TRUE}
summary(plants_mer_testme)$coef
```
...
## Posthocs from lmerTest

```{r posthoc, echo=TRUE}
lsmeans(plants_mer_nochamber)
```
...
\
\
```{r rikz_varslopeint_nocov, echo=TRUE}
...
## Testing the vcov assumption
```{r test_vcov, echo=TRUE}
anova(rikz_varslopeint, rikz_varslopeint_nocov, refit=FALSE)
```

## Look at al of those variances
```{r show_varint}
knitr::kable(broom::tidy(rikz_varslopeint))
```

## Testing Variable Intercept assumption
```{r test_varint, echo=TRUE}
anova(rikz_varslopeint, rikz_varslope, refit=FALSE)
```


## Testing Variable Slope assumption
...
```{r all_uncertain, echo=TRUE, eval=TRUE, cache=TRUE}
