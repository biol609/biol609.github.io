...
![](./images/07/cats_repeated_measurs.jpg)
...
## Pseudoreplication
![](./images/07/kinds_of_pseudoreplication.jpg){width=60%}
...
```{r mussel_hist}
mussels <- read.csv("lectures/data/07/mussels.csv")
...
## Random Effects
- A *random effect* is a parameter that varies across groups following a distribution  
\
...
```{r unbalanced}
mussels_ragged <- read.csv("lectures/data/07/mussels_ragged.csv")
...
## A Visual Explanation: Fixed Effects
...
## A Visual Explanation: Random Effects
...
## In R....
\
![](./images/07/lmer_lemur_tim_doherty.jpg){width=55%}
...
```{r lmer_quantiles, echo=TRUE, warning=FALSE, error=FALSE}
library(DHARMa)
...
```{r sjp_fe, echo=TRUE, eval = FALSE}
fixed_df <- predictInterval(mussels_mer, which = "fixed", 
                            newdata = test_data,
                           level = 0.95, include.resid.var = FALSE) %>%
  cbind(test_data)

#plot
ggplot(fixed_df, aes(x = site, y = fit, ymin = lwr, ymax = upr)) +
  geom_pointrange() + coord_flip() +
  geom_hline(yintercept = 0, color = "red", lty = 2)
```
...
\
- Bonus, try `glmmTMB` and search out needed functions