...
![](./images/04/hamilton_weight.jpg){width="80.00000%"}\
...
## SQUID!
![](./images/04/loligo_orig.jpeg)
```{r}
squid_summary <- read.delim("./data/04/squid_summary.txt", sep = " ")
```
...
```{r read_squid}
squid <- read.delim("./data/04/Squid.txt") %>%
  na.omit()
...
- Need to choose determine in what direction variance increases  
  - Weight by X or 1/X
\
...
\
- BUT - also has likelihood-based methods to git WLS and GLS  
\
...
squid_gls <- gls(Testisweight ~ DML, data=squid,
                 weights = varFixed(~ 1/DML))
```

Note: higher value = more weight, so, 1/DML
...
squid_gls_month <- gls(Testisweight ~ DML, data=squid,
                       ...
                       weights = varComb(varFixed(~1/DML),
