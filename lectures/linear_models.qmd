...
plot(density(prior_h)$x, 
     density(prior_h)$y/sum(density(prior_h)$y), xlab = "Height (cm)",
     type = "l", ylab = "density")
matplot(density(Howell1_Adult$height)$x,
      density(Howell1_Adult$height)$y/sum(density(Howell1_Adult$height)$y), 
     col = "red", add = TRUE, type = "l")
```
...
Prior:  
$\alpha   \sim \mathcal{N}(150, 100)$  &nbsp; <span class="fragment" style = "color:red">Previous Mean</span>  
...
```{r sim_prior0}
#| code-line-numbers: "|1-5|8-10|11-15"
#| eval: true
#| echo: true
...
# Or, Use `ggdist`
```{r fit_interval_data, echo=TRUE, eval=FALSE}
library(ggdist)
ggplot() +
...
```{r pred_interval_data, echo=TRUE, eval=FALSE}
