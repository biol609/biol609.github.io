...
\
![](./images/bayesian_glm/star_wars_glm.jpg){width="75%"}
...
## A Simple Model
```{r kline_simp, eval = FALSE}
kline_mod <- alist(
  #likelihood
  total_tools ~ dpois(lambda)
  
  #DGP
  log(lambda) <- a + b*population
...
## What should our priors be? Data Logged
```{r kline_data}
ggplot(Kline,
       aes(x = log(population), y = total_tools)) +
  geom_point()
```


## What should our priors be? Mean Tool Use
```{r kline_data2}
ggplot(Kline,
       aes(x = log(population), y = mean_TU)) +
  geom_point()
```
...
b_priors <- crossing(m = 0:3, 
  ...
  #make the line
  crossing(pop = seq(1,1e4, length.out=100)) %>%
  mutate(tools_sim = exp(value*pop))
```

## What about b?
```{r}
ggplot(b_priors,
       aes(x = log(pop), y = log(tools_sim), group = value)) +
  geom_line() +
  facet_grid(m ~ s, scale = "free")
```
