...
zip_fit <- rethinking::map(zip_mod, data = monks)
```
...
Feel free to look at postcheck or other diagnostics here to see if we're ok. And, note, when the two parts of this relationship become trickier, you will need to resort to `map2stan`.
...
fit_zib <- map(mod_zib, data = Fish %>% mutate(size = 1, with_kids = as.numeric(with_kids)))
```
...
```{r zib_bayes}
#input data frame
pred_df <- crossing(persons = seq(1,4,length.out=100), with_kids = c(1,2), size = 100)

#get predictions with error
out <- sim(fit_zib, pred_df)

#the mean fit
pred_df <- pred_df %>%
  mutate(percent_chance_of_fish = colMeans(out),
         with_kids = as.logical(with_kids-1))

#visualize
FishPlot +
  geom_line(data = pred_df, aes(y = percent_chance_of_fish/100),)
...
fit_zinb <- map(mod_zinb, data = Fish %>% mutate(size = 1, with_kids = as.numeric(with_kids)))
