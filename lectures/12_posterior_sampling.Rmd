...
grid <- data.frame(prob = seq(0,1,.01), prior=1) %>%
  ...
  mutate(posterior = likelihood*prob)
```
...
So, let's see it!
```{r quant, echo=FALSE}
#from http://stackoverflow.com/questions/3494593/shading-a-kernel-density-plot-between-two-points
dens <- density(samp)
q25 <- quantile(samp, 0.25)
q75 <- quantile(samp, .75)
x1 <- min(which(dens$x >= q25))  
x2 <- max(which(dens$x <  q75))
plot(dens)
with(density(samp), polygon(x=c(x[c(x1,x1:x2,x2)]), y= c(0, y[x1:x2], 0), col="gray"))
```
